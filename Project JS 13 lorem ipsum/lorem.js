...
// const form = document.querySelector('.lorem-font')
...
// I did this myself using fetch and async/await!! YAYAYAYAY! Let's GOOOOOOO!!

const btn = document.querySelector('.btn')
const input = document.querySelector('#amount')
const loremText = document.querySelector('.lorem-text')

const generateParagraph = async function (paras) {
    const res = await fetch(`http://hipsum.co/api/?type=hipster-centric&paras=${paras}`);
...
btn.addEventListener('click', async (e) => {
    ...
    if (isNaN(paras) || paras < 0) {
        ...
        let paragraphs = await generateParagraph(paras);
        console.log(paragraphs);
        loremText.textContent = `${paragraphs}`;
    }
